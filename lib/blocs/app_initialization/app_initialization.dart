...
class AppInitializationBloc
  ...
  FutureOr<void> _onAppInitializeSetup(
    ...
    try {
      ...
      try {
        ...
        print(result.serviceRegistry?.serviceRegistryList.length);
        await mdmsRepository.writeToRegistryDB(result, sbox);
  ...
  Future<MdmsConfig> _loadOfflineData(
    ...
    List<AppConfiguration> configs =[];
    try{

    serviceRegistryList =  Hive.box<ServiceRegistry>('serviceRegistryBox').values.toList();
    configs =  Hive.box<AppConfiguration>('AppConfigurationBox').values.toList();
    
print(configs.first.languages);
